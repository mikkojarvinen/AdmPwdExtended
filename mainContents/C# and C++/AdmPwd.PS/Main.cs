...
namespace AdmPwd.PS
{
    ...
    public class FindExtendedRights : Cmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            List<string> objectsToAnalyze = DirectoryUtils.Search(filter, OUs[0].DistinguishedName, "distinguishedName",500);
            foreach (string dn in objectsToAnalyze)
            {
                SearchRequest rq=new SearchRequest(dn,string.Format("(distinguishedName={0})", dn),System.DirectoryServices.Protocols.SearchScope.Base,"ntSecurityDescriptor");
                ...
                foreach (SearchResultEntry sr in rsp.Entries)
                {
                    ...
                    ExtendedRightsInfo data = new ExtendedRightsInfo(dn);
                    foreach (ActiveDirectoryAccessRule ace in acl.GetAccessRules(true, false, typeof(System.Security.Principal.NTAccount)))
                    {
                        if ((ace.ActiveDirectoryRights & System.DirectoryServices.ActiveDirectoryRights.ExtendedRight) == System.DirectoryServices.ActiveDirectoryRights.ExtendedRight)
                        {
                            ...
                            if(!data.ExtendedRightHolders.Contains(holder))
                                data.ExtendedRightHolders.Add(holder);
                        }
                    }
                    if (data.ExtendedRightHolders.Count > 0)
                        WriteObject(data);
                }
